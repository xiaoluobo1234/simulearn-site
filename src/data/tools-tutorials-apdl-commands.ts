...
> 📝 **相关练习**：[ex-apdl-command-01] 正确使用续行符
`,
...
> 📝 **相关练习**：[ex-apdl-macro-01] 创建带参数检查的宏文件
`,
...
后处理器 POST1 默认在全局直角坐标系中显示节点的位移、应力、应变等结果。但在工程实践中，结果的方向含义取决于观察坐标系——对于圆柱形结构，我们关心的是径向应力和周向应力，而不是 X 方向和 Y 方向的应力分量。\`RSYS\` 命令正是为解决此问题而设计的。

### RSYS 基本语法

\`RSYS, KCN\` — 将结果旋转到指定坐标系中显示。KCN 为坐标系编号：
...
切换 RSYS 后，所有后续的后处理显示和列表命令都将在新坐标系中输出。已储存的结果数据本身不会变化——ANSYS 在显示时实时进行坐标变换。

### RSYS 对各结果类型的影响

并非所有结果分量都会受 RSYS 影响。下表总结了主要结果类型的行为：

| 结果类型 | 命令示例 | 受 RSYS 影响？ | 说明 |
|---------|---------|--------------|------|
| 位移分量 (UX, UY, UZ) | \`PLNSOL,U,X\` | ✅ 是 | 位移分量旋转到新坐标系 |
| 位移矢量和 (USUM) | \`PLNSOL,U,SUM\` | ❌ 否 | 标量，与坐标系无关 |
| 正应力 (SX, SY, SZ) | \`PLNSOL,S,X\` | ✅ 是 | 应力张量分量旋转 |
| 剪应力 (SXY, SYZ, SXZ) | \`PLNSOL,S,XY\` | ✅ 是 | 剪应力分量也重新定向 |
| von Mises 等效应力 (SEQV) | \`PLNSOL,S,EQV\` | ❌ 否 | 应力不变量 |
| 主应力 (S1, S2, S3) | \`PLNSOL,S,1\` | ❌ 否 | 主应力及其方向余弦向量不变 |
| 应力强度 (SINT) | \`PLNSOL,S,INT\` | ❌ 否 | 应力不变量 |
| 弹性应变分量 | \`PLNSOL,EPEL,X\` | ✅ 是 | 与应力分量类似 |
| 反力 (RFOR) | \`PRRSOL,F\` | ⚠️ 注意 | 反力分量的方向定义取决于施加约束时的节点坐标系，不随 RSYS 改变 |

关键理解：\`RSYS\` 只改变分量结果的观察方向，不改变不变量（标量）的数值。如果你想验证这一点，可以在不同 RSYS 下分别打印 SX 和 SEQV——SX 的数值会变，SEQV 不变。

### 柱坐标系下的应力分量

切换到柱坐标系（\`RSYS,1\`）后，应力分量的物理含义变为：
...
PLNSOL, S, X                  ! 径向应力 σr（沿半径方向）
PLNSOL, S, Y                  ! 周向应力 σθ（沿切线方向）
PLNSOL, S, Z                  ! 轴向应力 σz（沿 Z 轴方向）
PLNSOL, S, XY                 ! 面内剪应力 τrθ（径向-周向剪切）
PLNSOL, S, YZ                 ! τθz
PLNSOL, S, XZ                 ! τrz
~~~

对于压力容器、管道和旋转机械，这种分量分解具有直接的工程意义：

| 柱坐标分量 | RSYS,1 下的标签 | 工程意义 |
|-----------|----------------|---------|
| SX | 径向应力 σr | 沿壁厚方向的应力分布，内壁最大 |
| SY | 周向应力 σθ | 环向应力（hoop stress），内压容器的控制应力 |
| SZ | 轴向应力 σz | 沿轴线方向的应力 |
| SEQV | von Mises 应力 | 判断屈服的标量准则 |

~~~apdl
! === 压力容器筒体应力分解 ===
RSYS, 1                       ! 柱坐标系

! 提取内壁最大周向应力（控制壁厚设计的关键参数）
*GET, hoop_max, PLNSOL, S, Y, 0, MAX

! 提取壁厚方向的径向应力梯度
/OUTPUT, radial_path, txt
PATH, radial_p, 2, 30, 20     ! 定义沿壁厚的路径
PPATH, 1, , 100               ! 内壁节点
PPATH, 2, , 200               ! 外壁节点
PDEF, sig_r, S, X             ! 映射径向应力
PDEF, sig_theta, S, Y         ! 映射周向应力
PLPATH, sig_r, sig_theta      ! 绘制应力沿壁厚分布曲线
/OUTPUT
~~~

上述代码展示了如何利用柱坐标系提取压力容器壁厚方向的应力分布——这是压力容器设计中必需的分析步骤。

### 局部坐标系与倾斜面结果

RSYS 的强大之处在于可以配合局部坐标系，提取任意方向的应力分量。这在分析焊缝、螺栓连接、复合材料层合板等需要重点关注特定方向的场景中非常实用：
...
! 创建对齐到焊缝方向的局部坐标系
! 假设焊缝与全局 X 轴夹角 30 度
LOCAL, 20, 0, WELD_X, WELD_Y, WELD_Z, 0, 0, 30

! 激活该局部坐标系
RSYS, 20

! 提取沿焊缝方向和垂直焊缝方向的应力
PLNSOL, S, X                  ! 此时 X 方向 = 沿焊缝方向
PLNSOL, S, Y                  ! 此时 Y 方向 = 垂直焊缝方向
PLNSOL, S, XY                 ! 焊缝面内的剪应力

! 对于螺栓连接，可以创建对齐到螺栓轴线的坐标系
! 以提取轴向力和剪切力
LOCAL, 21, 0, BOLT_CX, BOLT_CY, BOLT_CZ
RSYS, 21
PLNSOL, S, Z                  ! 螺栓轴向应力
~~~

**复合材料层合板的材料主方向分析**：复合材料的失效准则是基于材料主方向（纤维方向、垂直于纤维方向）定义的。使用与铺设方向对齐的局部坐标系，可以直接评估 Tsai-Wu、Tsai-Hill 等失效准则所需的应力分量。

### 位移结果的坐标系变换

当 RSYS 激活柱坐标系或球坐标系时，位移分量的含义也随之变化：

~~~apdl
RSYS, 1                       ! 柱坐标系
PLNSOL, U, X                  ! 径向位移 Ur（向外为正）
PLNSOL, U, Y                  ! 周向位移 Uθ（逆时针为正）
PLNSOL, U, Z                  ! 轴向位移 Uz

PLNSOL, U, SUM                ! 总位移（标量，不变）
~~~

径向位移在配合/过盈分析、旋转机械间隙评估中非常重要。周向位移在扭转分析和齿轮啮合分析中用来量化扭转角。

### RSYS 与后处理命令配合

RSYS 不仅影响图形显示，也影响所有基于方向分量的文本输出和参数提取命令：
...
! 列表输出——分量含义遵循当前 RSYS
PRNSOL, S, COMP               ! 列出 SX, SY, SZ, SXY, SYZ, SXZ（已旋转到柱坐标）
PRNSOL, U, COMP               ! 列出 UX, UY, UZ（已旋转）

! PRRSOL 列出反力——注意：反力不受 RSYS 影响
PRRSOL, F

! *GET 参数提取
*GET, sig_r_max, PLNSOL, S, X, 0, MAX      ! 最大径向应力
*GET, sig_theta_max, PLNSOL, S, Y, 0, MAX  ! 最大周向应力

! 单元表操作——也可以受益于 RSYS
ETABLE, , S, X                ! 单元表中 SX 也遵循当前 RSYS
ETABLE, , S, Y                ! SY 亦然
...
**特别注意**：\`PRRSOL\`（反力列表）和 \`FSUM\`（反力求和）不受 RSYS 影响。反力的方向由节点坐标系（NROTAT）控制，而非结果坐标系。这是实际工程中一个常见的困惑点。

### 综合诊断实例：压力容器过渡段分析

以下实例展示如何综合运用 RSYS 对压力容器筒体-封头过渡段进行应力分解和强度评估：
...
! 筒体周向应力（环向应力）应该是主要的承压应力
PLNSOL, S, Y
/TITLE, Circumferential (Hoop) Stress in Cylinder - MPa

! 提取最大周向应力并与理论值对比
*GET, hoop_FE, PLNSOL, S, Y, 0, MAX
! Lame 理论解：hoop_theory = P * (Ro^2 + Ri^2) / (Ro^2 - Ri^2)
! 对比 hoop_FE 与 hoop_theory，验证网格精度

! 检查径向应力——内壁应为 -P，外壁应为 0
PLNSOL, S, X
/TITLE, Radial Stress Distribution in Cylinder Wall

! === 步骤 2：封头过渡段的局部坐标系分析 ===
! 封头与筒体连接处存在弯曲应力，需要沿壁面法向和切向分解
! 创建与过渡段曲面法线对齐的局部坐标系
K, 1000, TRAN_X, TRAN_Y, TRAN_Z    ! 过渡段上的关键点
LOCAL, 30, 0, TRAN_X, TRAN_Y, TRAN_Z, , , 45
RSYS, 30
...
PPATH, 1, , INNER_NODE           ! 内壁节点
PPATH, 2, , OUTER_NODE           ! 外壁节点

PDEF, sig_hoop, S, Y             ! 周向应力沿路径分布
PDEF, sig_rad, S, X              ! 径向应力沿路径分布
PDEF, sig_axial, S, Z            ! 轴向应力沿路径分布

PLPATH, sig_hoop, sig_rad, sig_axial

! 应力线性化——提取膜应力 + 弯曲应力
! （需要配合 SINT 或 PRSECT 命令，此处展示基本思路）
...
**误区一：认为 RSYS 改变了结果数据本身。**
RSYS 只改变了结果的显示方向，不改变原始数据。在不同 RSYS 之间切换不会造成任何精度损失。

**误区二：在 RSYS 激活时用 \`*GET\` 提取反力。**
反力分量不受 RSYS 影响。要控制反力的方向，应在求解前用 \`NROTAT\` 旋转节点坐标系。

**误区三：忘记恢复 RSYS 为默认值。**
在需要回到全局直角坐标系查看结果时，务必执行 \`RSYS,0\`。尤其是在自动化脚本中，建议在每个后处理阶段开始时显式设置 RSYS：
...
**误区四：在不支持 RSYS 的后处理操作中使用。**
大多数 POST1 操作支持 RSYS，但某些高级操作（如单元表 \`ETABLE\` 中的某些量）的行为可能与预期不同。遇到异常时，先检查 \`RSYS,0\` 下的结果作为基准。

**误区五：混淆了 RSYS 与求解前坐标系。**
\`CSYS\` 用于建模和载荷施加，\`RSYS\` 用于结果观察。两者相互独立——使用柱坐标系建模（\`CSYS,1\`）并不影响后处理中的结果显示坐标系（默认 \`RSYS,0\`）。
...
## 本节要点
	...
	ANSYS 提供全局直角（\`CSYS,0\`）、柱（\`CSYS,1\`）和球（\`CSYS,2\`）三种全局坐标系。局部坐标系（\`LOCAL\`、\`CSKP\`、\`CSWPLA\`）可以放置在任意位置和方向。工作平面是辅助建模的移动参考平面，通过 \`WPCSYS\`、\`WPOFFS\`、\`WPROTA\` 操控。结果坐标系（\`RSYS\`）控制后处理中分量结果的方向解释——应力/位移分量旋转到观察坐标系，但不变量（von Mises、主应力）不受影响。柱坐标下的 SX/SY/SZ 分别对应径向/周向/轴向应力，这对于压力容器和旋转机械的强度评估至关重要。局部坐标系配合 RSYS 可提取任意方向的应力分量（焊缝、螺栓、复合材料）。注意区分：\`CSYS\` 用于建模和载荷施加，\`RSYS\` 用于后处理结果显示，两者相互独立。

> 📝 **相关练习**：[ex-apdl-coord-01] 在柱坐标系中创建扇形板
	`,
...
> 📝 **相关练习**：[ex-apdl-geom-01] 从关键点开始自底向上建模
`,
