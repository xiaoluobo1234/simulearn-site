...
当前生产 Worker：`b5c3c213-f006-4fc1-a5a7-b98d90ba5a49`
...
| 项目 | 当前状态 |
|---|---|
| Git | `main` 已推送到 `737f083`，与 `origin/main` 同步 |
| Cloudflare | Worker Static Assets 已部署到 `simulearn.cn` |
| 五领域课程 | 结构、热、流体、多物理场、芯片共 450 个预设知识点 |
| 工具教程课程 | 4 个系列、27 个主题章节、75 篇教程（Python 20 + APDL 25 + NumPy 15 + SciPy 15） |
...
| 当前工作区 | 仅有空的 `simulearn-scripts/` 嵌套仓库未跟踪，不得提交 |
...
原 20/30/40 三级混合路线已取消。当前公开 4 个系列、27 个主题章节、75 篇教程：
...
- Python 基础教程、APDL 初级教程、NumPy 数值计算教程、SciPy 科学计算教程四个 section；
- “仿真实践—案例整理中”占位；
- FEA 分析类型预判计算器；
- 独立脚本仓库链接。
...
- `toolsChapterOrder`：27 章 const 数组，控制渲染顺序和知识树分组；
- `tools.astro`：4 个 `<section>` 分别按 `p.series` 过滤渲染；
...
tools-tutorials-apdl-{foundation,commands,mesh-solve,post-advanced}.ts  → APDL 内容
...
```text
src/pages/tools.astro                                    工具首页（4 section 渲染）
src/pages/index.astro                                    首页
src/pages/domains/kp.astro                               知识点页（slug 前缀检测标签）
src/components/KnowledgeTree.astro                       左侧知识树（27 章分组）
src/data/tools-learning.ts                               元数据、seeds、chapterOrder、*KnowledgePoints
src/data/tools-tutorials.ts                              教程内容聚合器（12 个 import）
...
tests/catalog.test.ts                                    单元测试（75 篇断言）
tests/e2e/learning.spec.ts                               E2E 测试（27 章 / 75 卡片）
```

`tools-learning.ts` 保存教程元数据、章节顺序、难度和前置关系。12 个 `tools-tutorials-*.ts` 文件保存详细 Markdown 正文。

### 5.4 工具教程页面行为

工具教程复用通用知识点页，但进入 `domain === “tools”` 时：

- 左侧树显示 27 个主题章节和 75 篇教程；
- 右侧标题显示”知识教程”；
...
原”查看十年路线”按钮已替换为”工具脚本”；原”AI 知识库”英雄按钮已移除。

说明：AI 管理页面、资源菜单和页脚中的 AI 入口仍存在。本次用户只要求调整首页英雄按钮，不要误删整个 AI 管理系统。
...
1f5f74c  fix: upgrade CI to Node 24 and actions v5 to resolve npm ci failure
a6fc3f2  feat: 重构工具脚本基础教程
db532b0  feat: 接入工程书库并扩展工具学习内容
21cf5fe  feat: 加固学习系统并补充自动化验证
```
...
Version ID: b5c3c213-f006-4fc1-a5a7-b98d90ba5a49
Source commit: 737f083
```
...
- 普通知识点树、校订状态和知识关系；
- 首页工具脚本按钮与 AI 英雄按钮移除；
- 工具页 27 个章节、75 个卡片和仿真实践占位；
- 工具页四个系列标题（Python / APDL / NumPy / SciPy）可见；
- 工具教程正文；
...
首页英雄区包含 工具脚本 / 工程书库
GET /tools/                                200
工具页包含 27 个章节、75 个教程卡片
GET /api/knowledge/tools/python-intro      200
tutorialMode = true
difficulty = 零基础
practiceStatus = collecting
GET /api/knowledge/tools/numpy-intro       200
tutorialMode = true, series = numpy
GET /api/knowledge/tools/scipy-intro       200
tutorialMode = true, series = scipy
```
...
当前生产提交是 737f083，Worker 版本是
b5c3c213-f006-4fc1-a5a7-b98d90ba5a49。

五个仿真领域已有 450 个固定预设知识点。工具教程模块已完成 4 个系列
共 75 篇教程（Python 20 + APDL 25 + NumPy 15 + SciPy 15），按 27 个
主题章节组织。工具教程不能恢复初/中/高级混排，不能恢复 AI 拓展、
AI 问答或 AI 检验。仿真实践当前必须保持”案例整理中”，等待用户提供
真实案例。
...
当前最高优先级是补充真实仿真实践和教程人工校订。后续教程扩展参考
AGENT-HANDOFF.md 第 9 节。不要泄漏 Secret，不要复制第三方教程正文。
